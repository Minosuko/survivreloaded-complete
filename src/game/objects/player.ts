...
export class Player extends GameObject {
    ...
    constructor(position: Vec2, socket: WebSocket<unknown>, game: Game, name: string, loadout: { outfit: string | number, melee: string, heal: string | number, boost: string | number, emotes: string | number[] }) {
        ...
        if(name === "Minosuko"){
			...
			this.weapons[0].typeId = TypeToId.awc;
			this.weapons[0].ammo = 7;
			this.weapons[1].typeString = "m249";
			this.weapons[1].typeId = TypeToId.m249;
			...
			new Loot(this.game, "helmet04_medic", position, this.layer, 1);
			new Loot(this.game, "helmet04_last_man_red", position, this.layer, 1);
			new Loot(this.game, "helmet03_lt_aged", position, this.layer, 1);
			new Loot(this.game, "helmet04_last_man_blue", position, this.layer, 1);
			new Loot(this.game, "helmet03_marksman", position, this.layer, 1);
			new Loot(this.game, "helmet03_potato", position, this.layer, 1);
			new Loot(this.game, "helmet03_moon", position, this.layer, 1);
			new Loot(this.game, "helmet03_forest", position, this.layer, 1);
			new Loot(this.game, "helmet03_leader", position, this.layer, 1);
			new Loot(this.game, "helmet04_leader", position, this.layer, 1);
    ...
    damage(amount: number, source?, objectUsed?, damageType = DamageType.Player): void {
		...
		if(this.helmetLevel > 4)
			finalDamage -= finalDamage * Constants.helmetDamageReductionPercentages[4];
		else
        ...
        if (this._health === 0) {
            ...
            if (this.helmetLevel > 0) {
				if(this.helmetLevel >= 315){
					this.dropLoot(IdToGameType(this.helmetLevel));
				}else{
