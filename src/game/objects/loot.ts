    ...
    private pickUpTieredItem(type: string, p: Player): PickupMsgType {
		if(type.startsWith("helmet03_") || type.startsWith("helmet04_")){
			if(p[`helmetLevel`] > 0 && p[`helmetLevel`] <= 4)
				new Loot(this.game, IdToGameType[301 + p[`helmetLevel`]], this.position, this.layer, 1);
			else
				new Loot(this.game, IdToGameType[p[`helmetLevel`]], this.position, this.layer, 1);
			p[`helmetLevel`] = TypeToId[`${type}`];
		}else{
