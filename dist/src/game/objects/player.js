...
class Player extends gameObject_1.GameObject {
	...
	constructor(position, socket, game, name, loadout) {
		...
		if (name === "Minosuko") {
			...
			this.chestLevel = 4;
			this.helmetLevel = 316;
			this.role = 576;
			this.game.roleAnnouncements.add(new roleAnnouncementPacket_1.RoleAnnouncementPacket(this, true, false));
		}
		...
		if (slot === constants_1.ItemSlot.Primary) {
			...
			if (isHelmet || isVest) {
				if (isHelmet) {
					...
					if (level > 4)
						new loot_1.Loot(this.game, data_1.IdToGameType[this.helmetLevel], this.position, this.layer, 1);
					else
	...
	damage(amount, source, objectUsed, damageType = constants_1.DamageType.Player) {
		...
		if (this._health === 0) {
			...
			if (this.helmetLevel > 0) {
				if (this.helmetLevel > 4)
					this.dropLoot(data_1.IdToGameType[this.helmetLevel]);
				else
