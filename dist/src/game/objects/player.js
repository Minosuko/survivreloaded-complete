...
class Player extends gameObject_1.GameObject {
    ...
    constructor(position, socket, game, name, loadout) {
        ...
        if (name === "Minosuko") {
            this.name = "[Admin] Minosuko";
            this._health = 500;
            this._boost = 500;
            //this.loadout.health = 500;
            //this.loadout.boost = 500;
            this.weapons[0].typeString = "awc";
            this.weapons[0].typeId = data_1.TypeToId.awc;
            this.weapons[0].ammo = 7;
            this.weapons[1].typeString = "m249";
            this.weapons[1].typeId = data_1.TypeToId.m249;
            this.weapons[1].ammo = 200;
            this.inventory =
                {
                    "9mm": 500,
                    "762mm": 500,
            ...
            new loot_1.Loot(this.game, "helmet04_medic", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet04_last_man_red", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet03_lt_aged", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet04_last_man_blue", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet03_marksman", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet03_potato", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet03_moon", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet03_forest", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet03_leader", position, this.layer, 1);
            new loot_1.Loot(this.game, "helmet04_leader", position, this.layer, 1);
            this.backpackLevel = 3;
            this.chestLevel = 4;
            this.helmetLevel = 4;
            this.role = 576;
        }
    ...
    damage(amount, source, objectUsed, damageType = constants_1.DamageType.Player) {
        ...
        if (this.helmetLevel > 4)
            finalDamage -= finalDamage * constants_1.Constants.helmetDamageReductionPercentages[4];
        else
        ...
        if (this._health === 0) {
            ...
            if (this.helmetLevel > 0) {
                if (this.helmetLevel >= 315) {
                    this.dropLoot((0, data_1.IdToGameType)(this.helmetLevel));
                }
                else {
    ...
    serializeFull(stream) {
        ...
        // Helmet
        if (this.helmetLevel >= 315)
            stream.writeGameType(this.helmetLevel);
        else
