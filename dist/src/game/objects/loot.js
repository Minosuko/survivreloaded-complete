...
class Loot extends gameObject_1.GameObject {
	...
	interact(p) {
		...
		else if (this.typeString.startsWith("helmet03_") || this.typeString.startsWith("helmet04_")) {
			result = this.pickUpTieredItem(this.typeString, p);
			playerDirty = true;
		}
	...
	canPickUpItem(p) {
		if (this.typeString.endsWith("scope")) {
			...
			return this.canPickUpTieredItem(this.typeString, p);
		}
		else if (this.typeString.startsWith("helmet")) {
	...
	pickUpTieredItem(type, p) {
		if (type.startsWith("helmet03_") || type.startsWith("helmet04_")) {
			if (p[`helmetLevel`] > 0 && p[`helmetLevel`] <= 4)
				new Loot(this.game, data_1.IdToGameType[301 + p[`helmetLevel`]], this.position, this.layer, 1);
			else
				new Loot(this.game, data_1.IdToGameType[p[`helmetLevel`]], this.position, this.layer, 1);
			p[`helmetLevel`] = data_1.TypeToId[`${type}`];
		}
		else {
